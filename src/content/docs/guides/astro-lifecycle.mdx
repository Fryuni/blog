---
title: 'Astro Project - When am I?'
description: "How to detect at which point in Astro's lifecycle your code is running."
draft: true
---
...
and _where_ you are running. Not all those cases can be diferentiated from all the others,
but some can.
...
If you are writing a remark plugin, you can check those variables inside your plugin
as well. You'll see that they are set as such:

While running `astro dev` the following variables are set:
...
While running `astro build` the following variables are set:
...
Remark plugins are not invoked during `astro sync`, `astro check` or `astro preview`. So
inside a remark plugin, `import.meta.env.PROD` and `import.meta.env.DEV` can be used to
determine you are building for production or development.
